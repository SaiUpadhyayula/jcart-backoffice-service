...
class ProductControllerTest {
    ...
    @Test
    fun `should return list of products when valid category id is given`() {
        val categoryId: Long = 1
        `when`(categoryRepository.findOne(categoryId))
                .thenReturn(Category(1, "GreetingCards", "Greeting Cards", false))
    ...
    @Test
    fun `should throw error when querying for product with invalid category`() {
        val categoryId: Long = 1
        `when`(categoryRepository.findOne(categoryId))
                .thenReturn(null)
        mockMvc.perform(get("/products/category/$categoryId"))
